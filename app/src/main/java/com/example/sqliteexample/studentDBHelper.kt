...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
    ...
    fun createStudent(student: StudentModel): Long {
        ...
        return db.insert(
            DBContract.UserEntry.TABLE_NAME, null, values
        )
    }
    ...
    fun searchStudentByName(name: String): ArrayList<StudentModel> {
        ...
        try {
            cursor = db.rawQuery(
                "select * from " + DBContract.UserEntry.TABLE_NAME + " WHERE " + DBContract.UserEntry.COLUMN_NAME + " LIKE \"%" + name + "%\"",
                null
            )
        } catch (e: SQLiteException) {
            // if table not yet present, create it
    ...
    fun updateStudent(student: StudentModel): Int {
        ...
        return db.update(
            DBContract.UserEntry.TABLE_NAME, values, selection, selectionArgs
        )
    }
